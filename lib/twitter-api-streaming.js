...
function startObservingQueries() {
  if (allowedObserve) {
    ...
    if (client) {
      ...
      queryObserver = Collection.tp_tweetQueries.find({}, {limit: 1}).observe({
        added: function(doc) {
...
// Auto-streaming
Meteor.startup(() => {
  Collection.ServerSettings.find({'apiTokens.twitter.isReady': true}, {fields: {'apiTokens.twitter': 1}, limit: 1}).observe({
    added: function(doc) {
