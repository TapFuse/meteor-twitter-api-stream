...
Collection.tp_tweetCache.after.insert(function(userId, doc) {
  doc.isDeleted = false;
  if (Collection.tp_tweetCache.find({queryId: doc.queryId}).count() >= 150) {
    const oldest = Collection.tp_tweetCache.findOne({queryId: doc.queryId}, {sort: {createdAtFormatted: 1}});
    if (oldest) {
